        ...
        <ul className='flex w-[22rem] flex-wrap items-center justify-center gap-y-1 text-[0.9rem] font-medium text-gray-500 sm:w-[initial] sm:flex-nowrap sm:gap-5'>
          {links.map((link) => (
            ...
            >
              <Link
                href={`${link.hash}`}
                // check if active section matches link name
                className={clsx(
                  'flex w-full items-center justify-center bg-transparent p-3 transition hover:text-gray-950',
              ...
              >
                {link.name}
                {/* check if current section matches link name */}
